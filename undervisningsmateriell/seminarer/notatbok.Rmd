...
tedtalks_subset %>%
  ...
  layout(xaxis = list(title = ""), # Ønsker ikke at det skal være tekst på y-eller x-aksen
         ...
         plot_bgcolor = "black",
         paper_bgcolor = "black",
         font = list(color = "lightgray")) # Sorter søylene i synkende rekkefølge ift. y-aksen (altså antallet)
...
# Estimere STM-modell
virkord_stm <- stm(virkord_dfm,                   # dfm objekt
...
load("./data/virkord_stm.rda")
```


Akkurat som før, kan vi få en kjapp oversikt over emnene modellen har funnet. Her vil det nok være mye likt som modellen over, men noen forskjeller kan vi se.

```{r plot_stm}
plot(virkord_stm, labeltype = "frex")
```

### Estimere effekt på emner

Det viktigste med `stm`-pakken, er likevel å utnytte `prevalence`-funksjonen. Her har vi estimert en modell som tar hensyn til forfatterens kjønn (dette var automatisk generert med `gender`-pakken, så feil kan forekomme). Fra plottet over, ser vi at emne 17 har sterk sammenheng med ordene "stemmeret" og "kvinde"; sannsynligvis tekster fra kampen om stemmerett for kvinner (derfor også dansk-norske ord). Via funksonen `estimateEffect()` kan vi se om det stort sett er kvinnelige forfattere som har _generert_ dette emnet:

```{r estimateeff}

stm_gender_eff <- estimateEffect(~ factor(gender),
                                 stmobj = virkord_stm,
                                 metadata = virkord)
...
Overraskende, sier modellen at menn har om man trekker en tilfeldig tekst innefor dette emnet, vil det være større sannsynlighet for å trekke en tekst med mannlig forfatter (dog med overlappende konfidensintervall). I en mer grundig analyse, ville vi her gått inn i emne 17 og sjekket veldig grundig hva det handlet om, sett på kjønnsvariabelen om det kan være noe feil i den, osv. Jeg kan røpe at denne effekten sannsynligvis er drevet av at det er ganske mange flere mannlige forfattere i korpuset generelt:

```{r meneverywhere}
table(virkord$gender)
```
