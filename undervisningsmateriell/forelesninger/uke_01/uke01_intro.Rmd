...
<p style='font-size:10;color:#24D4A7'>[01] Introduksjon</p></br>
...
text <- lapply(1:length(track_urls), function(x){
  ...
  tmp2 <- tmp %>% 
    html_elements(xpath = '//*[@id="lyrics-root"]/div[1]') %>% 
    html_text2() %>% 
    str_split("\\n") %>% 
    unlist() %>% 
    str_c(collapse = " ") %>% 
...
```{r nettverk, message=FALSE, warning = FALSE}
library(igraph)
library(ggraph)
...
gdat_twomode %>% 
  ...
  geom_node_point(aes(color = degree, 
                      size = degree)) + 
  geom_node_text(aes(label = name), color = "gray50") +
...
```{r ordbok, echo = TRUE, message=FALSE, eval=FALSE}
...
no4_tokens %>%
  ...
  ggdark::dark_theme_void()
ggsave("./triste_ord.png", width = 10, height = 10)

```

## {data-background=triste_ord.png data-background-size="5200px"}
 

<!-- ![](./triste_ord.png){width=98%} -->
 
```{r ordbok-ordsky, echo = FALSE, message=FALSE, eval=FALSE}
...
<!-- `convert sentiment_wordcloud_political.png -negate -sharpen 0x4 -shave 0x0+10+10 sent_wc_pol.png` -->
![](sent_wc_pol.png){width=60%}
...
Eksempel fra temamodellering i pensum [@Lucas2015]:

![](lucas_topics.PNG){width=50%}
...
<!-- `convert spektrum.PNG -negate -shave 0x10+0+0 spektrum2.png` -->
![](spektrum2.png){width=60%}


# Oppsummering

Tekst innebærer å jobbe i fem hovedbolker:

1. Forme og strukturere teksten (`stringr`)
2. Preprosessere teksten (`tidytext`)
3. Vektorisere teksten (`tidytext`)
4. Visualisere teksten (`ggplot`)
5. Analysere teksten
